            ...
            for elem in elements:
                if elem.name == 'h6':
                    ...
                    for link in found_links:
                        ...
                        if any('maxbutton' in c for c in classes): is_download_button = True
                        
                        if is_download_button or 'drive' in href or 'file' in href or 'cloud' in href:
                             # Combine quality into text for the bot's display
                             display_text = f"[{current_quality}] {text}"
                             links.append({'text': display_text, 'link': href})

        # Strategy 2: Fallback
        if not links:
            for a in soup.find_all('a', class_=lambda c: c and 'maxbutton' in c, href=True):
