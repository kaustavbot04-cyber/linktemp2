...
def bypass_gofile(url):
    ...
    try:
        # Step 1: Extract ID
        content_id = url.strip().split('/')[-1]
        
        # Step 2: Try Direct Worker Link
        # We try to see if it's a file by checking headers or just assuming valid if 200
        # If it returns 200 and Content-Type isn't HTML, it's likely a file.
        worker_link = f"https://gofile.dd-bypassed.workers.dev/{content_id}"
        
        try:
            head_resp = requests.head(worker_link, headers=headers, timeout=5)
            # If it's a direct file, Content-Type will be application/octet-stream, video/mp4 etc.
            # If it's a folder, the worker might return 404 or an HTML page.
            if head_resp.status_code == 200:
                c_type = head_resp.headers.get('Content-Type', '').lower()
                if 'text/html' not in c_type:
                    return worker_link
        except Exception:
            pass

        # Step 3: Try Worker API (Folder Handling)
        worker_api = f"https://gofile.dd-bypassed.workers.dev/api/{content_id}"
        try:
            api_resp = requests.get(worker_api, headers=headers, timeout=10)
            if api_resp.status_code == 200:
                data = api_resp.json()
                
                # Check for standard GoFile API structure
                if isinstance(data, dict) and data.get('status') == 'ok' and 'data' in data:
                    children = data['data'].get('children', {})
                    if children:
                        results = []
                        for child_id, child_data in children.items():
                            name = child_data.get('name', 'File')
                            # Construct worker link for child
                            child_link = f"https://gofile.dd-bypassed.workers.dev/{child_data.get('id', child_id)}"
                            results.append({'text': name, 'link': child_link})
                        return results
        except Exception as e:
            logging.error(f"GoFile Worker API Error: {e}")

        # Step 4: Fallback to Official API (Original Logic)
...
def get_download_links(url):
    ...
    if is_gofile_url(url):
        ...
        if isinstance(results, str):
            ...
            return msg
        return f"❌ Failed to extract GoFile: {url}"
    ...
    for item in links:
        ...
        if is_hubcloud_url(original):
            ...
            if isinstance(gf_results, str):
                 message += f"🎬 <b>{safe_quality} (GoFile Direct)</b>\n"
                 message += f"  └ 🔗 <a href='{gf_results}'>Click to Download</a>\n\n"
                 continue
            elif isinstance(gf_results, list):
...
async def perform_scrape_and_reply(url, update, context, reply_to_msg_id=None):
    chat_id = update.effective_chat.id
    
    # Determine which message to reply to
    target_msg_id = reply_to_msg_id
    if not target_msg_id and update.message:
...
async def perform_google_search(query_text, update, context, reply_to_msg_id=None):
    try:
        ...
        if not results:
            await context.bot.send_message(
                chat_id=update.effective_chat.id, 
                text=f"❌ No Google results found for '{html.escape(query_text)}'.",
                reply_to_message_id=target_msg_id
            )
            return
        ...
        logging.error(f"Google Search Error: {e}")
        # Recalculate ID in case it wasn't set earlier
        target_msg_id = reply_to_msg_id or (update.message.message_id if update.message else None)
        await context.bot.send_message(
            chat_id=update.effective_chat.id, 
            text="❌ Error performing Google search.",
            reply_to_message_id=target_msg_id
        )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_text.startswith("http"):
        ...
        for url in urls:
            if not url: continue
            # Pass the current message ID to reply to
            await perform_scrape_and_reply(url, update, context, reply_to_msg_id=update.message.message_id)
        return
    ...
    try:
        ...
        callback_data_google = f"google|{user_id}|{search_id_google}|{message_id}"
...
async def handle_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("google|"):
        ...
        # Determine parts based on length (legacy vs new)
        if len(data_parts) >= 4:
             _, owner_id, search_id, msg_id_str = data_parts
             owner_message_id = int(msg_id_str)
        elif len(data_parts) == 3:
            _, owner_id, search_id = data_parts
        else:
            await query.answer(text="Invalid data", show_alert=True)
            return
        ...
        if cached_data and isinstance(cached_data, dict) and cached_data.get('type') == 'google_search':
            ...
            await perform_google_search(user_query, update, context, reply_to_msg_id=owner_message_id)
            return
        else:
    ...
    if len(data_parts) == 3:
        search_id, owner_id, msg_id_str = data_parts
        owner_message_id = int(msg_id_str)
        if int(owner_id) != query.from_user.id:
    ...
    # Pass the original message ID (owner_message_id) so the result replies to the user's command
    await perform_scrape_and_reply(url, update, context, reply_to_msg_id=owner_message_id)
