    ...
    try:
        ...
        if api_resp.status_code == 200 and api_resp.headers.get('content-type') and 'json' in api_resp.headers['content-type']:
             try:
                 ...
                 results = []
                 # The worker's API structure is expected to return a list of items
                 if isinstance(api_data, list):
                     ...
                     if results:
                        return results
                 # Handling the old worker structure from the original code (just in case)
                 elif isinstance(api_data, dict) and api_data.get('status') == 'ok' and 'data' in api_data:
                    children = api_data['data'].get('children', {})
    ...
    # 4. Fallback to Original GoFile API (for content information)
        ...
        if content_resp.status_code == 200:
            ...
            if content_data['status'] == 'ok':
                if content_data['data'].get('type') == 'file' and content_data['data'].get('link'):
                     # If it's confirmed to be a single file, return the worker link
                     return worker_url
                
                # Folder structure using the official API (if worker API failed)
                items = content_data['data'].get('children', {})
                results = []
                for item_id, item in items.items():
                    # Construct worker link for child
                    child_id = item.get('id', item_id)
                    child_link = f"https://gofile.dd-bypassed.workers.dev/{child_id}"
                    name = item.get('name')
                    if child_link:
                        results.append({'text': name, 'link': child_link})
                if results:
...
def scrape_cinevood(url):
    ...
    try:
        ...
        # Using logic from snippet for improved parsing
        for elem in content_div.find_all(['h5', 'h6', 'a']):
            if elem.name in ['h5', 'h6']:
                ...
                text = elem.get_text().strip()
                if not text: text = elem.get('title', '').strip()
